...
abstract class AbstractApiTransport implements TransportInterface
{
        ...
        foreach ($this->getAllowedOptions() as $name => $regex) {
            if (isset($options[$name])) {
                ...
                if (preg_match($regex, $value)) {
                    ...
                    throw new UltimateException("Option '$name' has invalid value");
                }
            }
        }
    }
    ...
    protected function sendViaFile($data)
    {
        ...
        $tmpfile = tempnam(sys_get_temp_dir(), 'ultimate');
    ...
    protected function getApiHeaders()
    {
        return [
            ...
            'X-Ultimate-Key' => $this->config->getIngestionKey(),
            'X-Ultimate-Version' => $this->config->getVersion(),
        ];
    }
}
